...
import { calculateMetrics, calculateOperationScore } from "./metrics.js?v=20260920a";
...
// ------------------------------------------------------------
// Resumo pra copiar — uma linha por BR, no formato:
//   BR / cidade / dias parados
// Segue exatamente o que está filtrado na tela (status, entregador,
// cidade, faixa de dias e a busca), e não só as 300 linhas mostradas.
// ------------------------------------------------------------
const btnBacklogCopiarResumo = document.getElementById("btnBacklogCopiarResumo");
let backlogVisibleRows = [];
let backlogFilterText = "";

function backlogDaysText(raw) {
  // mostra o valor como vem do arquivo (ex: 14.24), igual ao "Notificar Cidade"
  const valor = (raw == null ? "" : raw).toString().trim() || "0";
  return `${valor} ${parseFloat(valor) === 1 ? "dia parado" : "dias parados"}`;
}

// Mesmo desenho do resumo do "Notificar Cidade" (título, total e lista
// com "- "), mas funciona com qualquer filtro e cada linha é
// BR / cidade / dias parados.
function buildBacklogCopySummary(rows, filterText) {
  const total = rows.length;
  const critical = rows.filter((r) => agingRank(r["LM Leg Aging"]) === 3).length;
  const linhas = rows.map((r) => {
    const br = (r["Shipment ID"] || "—").toString().trim();
    return `- ${br} / ${r.__city || "Sem cidade"} / ${backlogDaysText(r["LM Leg Days"])}`;
  });

  return `Backlog em Aberto${filterText ? " - " + filterText : ""}
...
async function copyBacklogSummary() {
  if (!backlogVisibleRows.length) {
    toast("Nenhum pacote no filtro atual pra copiar", "warn");
    return;
  }
  const total = backlogVisibleRows.length;
  const ok = await copyTextToClipboard(buildBacklogCopySummary(backlogVisibleRows, backlogFilterText));
  if (ok) toast(`Resumo de ${total} pacote${total === 1 ? "" : "s"} copiado — já pode colar`, "good");
  else toast("Não foi possível copiar automaticamente", "bad");
}
...
function renderBacklogView() {
  ...
  if (!hasBacklog) {
    ...
    backlogTableBody.innerHTML = `<tr><td colspan="7" class="table-empty">Nenhum arquivo de Backlog carregado ainda — use o Atualizar Dados ou, na barra lateral, Backlog › Escolher arquivo(s).</td></tr>`;
    backlogVisibleRows = [];
    backlogFilterText = "";
    if (btnBacklogCopiarResumo) btnBacklogCopiarResumo.style.display = "none";
  ...
  // Guarda o que está filtrado agora (todas as linhas, não só as 300
  // mostradas) pro botão "Copiar resumo das BRs"
  backlogVisibleRows = filtered;
  backlogFilterText = [
    stationValue,
    cityValue,
    driverValue,
    statusValue,
    backlogAgingFilter !== null ? BACKLOG_AGING_LABELS[backlogAgingFilter] : "",
    term ? `busca "${term}"` : "",
  ]
    .filter(Boolean)
    .join(" · ");
  if (btnBacklogCopiarResumo) btnBacklogCopiarResumo.style.display = "inline-flex";
...
if (btnBacklogCopiarResumo) {
  btnBacklogCopiarResumo.addEventListener("click", copyBacklogSummary);
}
...
// ---- dados do cliente do pedido (nome e telefone) — o nome das colunas muda
// de arquivo pra arquivo, então procura por padrões ----
const CLIENTE_NOME_REGEX = [
  /^(buyer|customer|recipient|receiver|consignee)[\s_]*name$/i,
  /(buyer|customer|recipient|receiver|consignee|cliente|comprador|destinat[aá]rio).*(name|nome)/i,
  /^(buyer|customer|recipient|receiver|consignee|cliente|comprador|destinat[aá]rio)$/i,
];
const CLIENTE_FONE_REGEX = [
  /(buyer|customer|recipient|receiver|consignee).*(phone|tel|mobile|contact|cell)/i,
  /(phone|tel|mobile|contact|cell).*(buyer|customer|recipient|receiver|consignee)/i,
  /(cliente|comprador|destinat[aá]rio).*(telefone|fone|celular|contato|whats)/i,
];

function findClientField(row, regexes) {
  const keys = Object.keys(row || {});
  for (const rx of regexes) {
    const key = keys.find((k) => rx.test(k) && !/driver|motorista|entregador/i.test(k));
    if (key) return key;
  }
  return null;
}

// nome e telefone do cliente por pedido (Order ID), lidos dos arquivos de SLA/DS
function buildOrderCustomerMap() {
  const names = {};
  const phones = {};
  let nameKey = null;
  let phoneKey = null;
  [...slaRows, ...dsRows].forEach((r) => {
    if (nameKey === null) nameKey = findClientField(r, CLIENTE_NOME_REGEX) || false;
    if (phoneKey === null) phoneKey = findClientField(r, CLIENTE_FONE_REGEX) || false;
    const orderId = (r["Order ID"] || "").toString().trim();
    if (!orderId) return;
    if (nameKey && !names[orderId] && r[nameKey]) names[orderId] = r[nameKey].toString().trim();
    if (phoneKey && !phones[orderId] && r[phoneKey]) phones[orderId] = r[phoneKey].toString().trim();
  });
  return { names, phones };
}

// "JÂNIO CARLOS DA SILVA" -> "Jânio Carlos da Silva"
function toProperName(value) {
  return (value || "")
    ...
    .split(/\s+/)
    .filter(Boolean)
    .map((w, i) => (i > 0 && ["da", "de", "do", "das", "dos", "e"].includes(w) ? w : w.replace(/^\p{L}/u, (c) => c.toUpperCase())))
    .join(" ");
}

// 98991234567 -> (98) 99123-4567
...
function buildPnrMessage(pnr) {
  ...
  const br = pnr["SPXTN"] || "—";
  const driver = toProperName(pnr.__driverName) || "entregador";
  const client = toProperName(pnr.__clientName) || "(nome do cliente não informado)";
  const phone = formatPhoneBR(pnr.__clientPhone) || "(telefone do cliente não informado)";

  return `Caro entregador(a) ${driver}, você tem esta PNR pendente:
...
Peço que resolva o mais rápido para não virar desconto.

Para resolver, envie a seguinte mensagem ao cliente e obtenha a resposta:

*Olá! Confirmando a entrega do pedido ${br}. Você recebeu o produto? Responda apenas SIM ou NÃO.*

Tire print e anexe como evidência na sua PNR.
Obs: não pode salvar o contato e não pode ter foto de fundo na conversa; de preferência use o padrão do WhatsApp.`;
}
...
function renderPnrView() {
  ...
  if (!hasPnr) {
    ...
    pnrTableBody.innerHTML = `<tr><td colspan="7" class="table-empty">Nenhum arquivo de PNR carregado ainda — use o Atualizar Dados ou, na barra lateral, PNR › Escolher arquivo(s).</td></tr>`;
  ...
  const orderCityMap = buildOrderCityMap();
  const driverCityMap = buildDriverCityMap();
  const orderCustomers = buildOrderCustomerMap();
  const pnrNameKey = pnrRows.length ? findClientField(pnrRows[0], CLIENTE_NOME_REGEX) : null;
  const pnrPhoneKey = pnrRows.length ? findClientField(pnrRows[0], CLIENTE_FONE_REGEX) : null;
  let rows = pnrRows
    ...
    .map((r) => {
      ...
      return {
        ...
        __city: city,
        __clientName: (pnrNameKey && r[pnrNameKey] ? r[pnrNameKey].toString().trim() : "") || orderCustomers.names[(r["SPXTN"] || "").toString().trim()] || "",
        __clientPhone: (pnrPhoneKey && r[pnrPhoneKey] ? r[pnrPhoneKey].toString().trim() : "") || orderCustomers.phones[(r["SPXTN"] || "").toString().trim()] || "",
      };
    });
...
function buildReportForActiveView() {
  ...
  // GERAL: SLA e DS em blocos separados (cada um com as suas próprias contas).
  // Antes as linhas dos dois eram juntadas numa lista só e os números saíam
  // somados e misturados.
  return buildDeliveryFullReportGeral(sla, ds);
}

function buildDeliveryFullReportGeral(sla, ds) {
  const partes = [];
  const relatorioSla = buildDeliveryFullReport(sla, "SLA", "SLA");
  const relatorioDs = buildDeliveryFullReport(ds, "DS", "DS");
  if (!relatorioSla.isEmpty) partes.push({ rotulo: "SLA", ...relatorioSla });
  if (!relatorioDs.isEmpty) partes.push({ rotulo: "DS", ...relatorioDs });

  return {
    titleLine: "RELATÓRIO COMPLETO — GERAL (SLA & DS)",
    emptyMessage: "Nenhum dado de SLA ou DS carregado para gerar o relatório",
    isEmpty: partes.length === 0,
    partes,
  };
}
...
function exportPdfReport() {
  ...
  // Faixa que abre cada bloco do relatório Geral ("SLA" / "DS")
  function drawPartBanner(text) {
    ensureSpace(40);
    pdf.setFillColor(darkRgb[0], darkRgb[1], darkRgb[2]);
    pdf.rect(marginX, y - 6, usableWidth, 26, "F");
    pdf.setFont("helvetica", "bold");
    pdf.setFontSize(13);
    pdf.setTextColor(255, 255, 255);
    pdf.text(text, marginX + 10, y + 12);
    pdf.setTextColor(darkRgb[0], darkRgb[1], darkRgb[2]);
    y += 38;
  }

  // Desenha um relatório completo: resumo, status, cidades, entregadores.
  // No Geral são duas partes (SLA e DS), cada uma com os seus números.
  function drawReportPart(parte, sufixo) {
    const titulo = (base) => (sufixo ? `${base} — ${sufixo}` : base);

    // ---- Resumo (KPIs) ----
    drawSectionTitle(titulo("Resumo geral"));
    const cardW = usableWidth / parte.summary.length;
    ensureSpace(50);
    const cardY = y;
    parte.summary.forEach((card, i) => {
    ...
    if (parte.statusBreakdown && parte.statusBreakdown.length) {
      drawSectionTitle(titulo("Distribuição por status"));
      drawTable(
        ["Status", "Quantidade"],
        [0.7, 0.3],
        parte.statusBreakdown.map((s) => ({ cells: [s.label, String(s.count)], bad: false }))
      );
    }

    // ---- Detalhamento por cidade ----
    drawSectionTitle(titulo("Detalhamento por cidade"));
    if (parte.cityRows.length) {
      drawTable(parte.cityColumns, parte.cityWidths, parte.cityRows);
    } else {
      ensureSpace(16);
    ...
    drawSectionTitle(titulo("Detalhamento por entregador"));
    drawTable(parte.driverColumns, parte.driverWidths, parte.driverRows);
    ...
    if (parte.legendNote) {
      ...
      pdf.text(parte.legendNote, marginX, y);
      y += 24;
    }
  }
  ...
  if (report.partes) {
    // GERAL: um bloco pra cada (SLA, DS), cada um começando numa página nova
    report.partes.forEach((parte, i) => {
      if (i > 0) {
        pdf.addPage();
        drawHeaderBar();
      }
      drawPartBanner(parte.rotulo);
      drawReportPart(parte, "");
    });
  } else {
    drawReportPart(report, "");
  }

  const stationLabel = stationSelect && stationSelect.value ? stationSelect.value.replace(/[^a-zA-Z0-9]+/g, "-") : "todos";
  const reportSlug = report.titleLine.split("—")[1]?.trim().toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-+|-+$/g, "") || "geral";
...
//   tempo em rota  = da hora que subiu a rota até agora
//   produtividade  = (pacotes entregues por hora em rota) ÷ meta
// Meta: 30 pacotes por hora = 100%. Acima disso é muito bom,
// abaixo é ruim. É só mudar os números aqui embaixo pra ajustar.
// ------------------------------------------------------------
const META_PACOTES_POR_HORA = 30;         // 100% = 30 pacotes por hora
const PRODUTIVIDADE_MINIMO_MIN = 15;      // com menos de 15 min de rota ainda não dá pra medir
const PRODUTIVIDADE_ATENCAO = 70;         // de 70% a 99% = regular (laranja); abaixo de 70% = ruim (vermelho)
const PARAR_RELOGIO_AO_FINALIZAR = true;  // rota finalizada: o tempo em rota para na última entrega
                                          // (senão um entregador que acabou cedo iria "piorando" o dia todo)

function classificarProdutividade(produtividade, pacotesPorHora) {
  if (produtividade === null || produtividade === undefined) {
    return {
      classe: "prod-nd",
      texto: "-",
      extra: "",
      dica: `Ainda sem dados suficientes (precisa de pelo menos ${PRODUTIVIDADE_MINIMO_MIN} min em rota)`,
    };
  }
  const classe = produtividade >= 100 ? "prod-otima" : produtividade >= PRODUTIVIDADE_ATENCAO ? "prod-media" : "prod-ruim";
  const porHora = pacotesPorHora.toFixed(1).replace(".", ",");
  return {
    classe,
    texto: `${Math.round(produtividade)}%`,
    extra: `${porHora}/h`,
    dica: `${porHora} pacotes por hora em rota (meta: ${META_PACOTES_POR_HORA}/h = 100%)`,
  };
}

function buildDriverActivityRows(rows, agora) {
  const byDriver = {};
  (rows || []).forEach((row) => {
  ...
  return Object.values(byDriver)
    .map((entry) => {
      ...
      // Tempo em rota: da hora que subiu até agora (ou até a última entrega,
      // se já finalizou a rota)
      const fimDaRota = PARAR_RELOGIO_AO_FINALIZAR && status === "Finalizado" && entry.lastDelivered ? entry.lastDelivered : now;
      const tempoEmRotaMs = entry.pickup ? Math.max(0, fimDaRota - entry.pickup) : null;

      let pacotesPorHora = null;
      let produtividade = null;
      if (tempoEmRotaMs !== null && tempoEmRotaMs >= PRODUTIVIDADE_MINIMO_MIN * 60000) {
        pacotesPorHora = entry.deliveredCount / (tempoEmRotaMs / 3600000);
        produtividade = (pacotesPorHora / META_PACOTES_POR_HORA) * 100;
      }
      ...
      return {
        ...
        tempoEmRotaMs,
        tempoEmRotaLabel: tempoEmRotaMs !== null ? formatDuration(tempoEmRotaMs) : "-",
        pacotesPorHora,
        produtividade,
...
function renderDriverActivityTable(rows) {
  ...
  tbody.innerHTML = filtered
    .map((r, idx) => {
      ...
      const prod = classificarProdutividade(r.produtividade, r.pacotesPorHora);
      ...
      return `<tr>
        ...
        <td>${r.tempoEmRotaLabel}</td>
        <td class="prod-cell"><span class="prod-badge ${prod.classe}" title="${prod.dica}">${prod.texto}</span>${prod.extra ? `<small>${prod.extra}</small>` : ""}</td>
...
// ------------------------------------------------------------
// Rankings da Análise de OP — 10 melhores e 10 piores por
// entregador (SLA e DS). Soma os dias do período (cada dia entra
// com o próprio resultado, igual ao gráfico) e respeita os filtros
// de cidade/entregador da aba.
// ------------------------------------------------------------
const OP_RANKING_TOP = 10;          // quantos aparecem em cada tabela
const OP_RANKING_MIN_PACOTES = 10;  // mínimo de pacotes no período pra entrar no ranking

function escaparHtmlOp(texto) {
  return String(texto)
    .replace(/&/g, "&amp;")
    .replace(/</g, "&lt;")
    .replace(/>/g, "&gt;")
...
// Soma, por entregador, os totais de um dia no acumulado do período
function acumularRankingOp(acum, driverSLA) {
  (driverSLA || []).forEach((d) => {
    const nome = (d.name || "").toString().trim();
    if (!nome) return;
    if (!acum[nome]) acum[nome] = { total: 0, delivered: 0 };
    acum[nome].total += d.total;
    acum[nome].delivered += d.delivered;
  });
}

function montarRankingOp(acum, ignorarMinimo) {
  const lista = Object.entries(acum || {})
    .map(([name, s]) => ({
      name,
      total: s.total,
      delivered: s.delivered,
      pending: s.total - s.delivered,
      sla: s.total > 0 ? (s.delivered / s.total) * 100 : 0,
    }))
    .filter((r) => r.total > 0 && (ignorarMinimo || r.total >= OP_RANKING_MIN_PACOTES));

  const porNome = (a, b) => a.name.localeCompare(b.name, "pt-BR");
  // desempate: quem tem mais pacotes fica na frente
  const melhores = [...lista]
    .sort((a, b) => b.sla - a.sla || b.total - a.total || porNome(a, b))
    .slice(0, OP_RANKING_TOP);
  const piores = [...lista]
    .sort((a, b) => a.sla - b.sla || b.total - a.total || porNome(a, b))
    .slice(0, OP_RANKING_TOP);

  return { melhores, piores };
}

function preencherTabelaOpRanking(tbodyId, lista, comMedalha, mensagemVazia) {
  const tbody = document.getElementById(tbodyId);
  if (!tbody) return;

  if (!lista.length) {
    tbody.innerHTML = `<tr><td colspan="6" class="table-empty">${mensagemVazia}</td></tr>`;
    return;
  }
  ...
  tbody.innerHTML = lista
    .map((r, i) => {
      const nome = escaparHtmlOp(r.name);
      return `<tr>
        <td class="op-rank-pos">${comMedalha && i < 3 ? medalhas[i] : i + 1}</td>
        <td title="${nome}">${nome}</td>
        <td>${r.total}</td>
        <td>${r.delivered}</td>
        <td>${r.pending}</td>
        <td class="${slaClass(r.sla)}">${r.sla.toFixed(2)}%</td>
      </tr>`;
    })
    .join("");
}

function renderOpRankings(acumSla, acumDs, driverFiltrado) {
  // Com um entregador escolhido no filtro, o ranking é só ele — então
  // o mínimo de pacotes não pode esconder ele.
  const ignorarMinimo = !!driverFiltrado;
  const temDados = (acum) => Object.keys(acum || {}).length > 0;

  const sla = montarRankingOp(acumSla, ignorarMinimo);
  const ds = montarRankingOp(acumDs, ignorarMinimo);

  const vazio = (acum) =>
    temDados(acum) && !ignorarMinimo
      ? `Nenhum entregador com pelo menos ${OP_RANKING_MIN_PACOTES} pacotes no período`
      : "Sem dados no período";

  preencherTabelaOpRanking("opRankSlaBest", sla.melhores, true, vazio(acumSla));
  preencherTabelaOpRanking("opRankSlaWorst", sla.piores, false, vazio(acumSla));
  preencherTabelaOpRanking("opRankDsBest", ds.melhores, true, vazio(acumDs));
  preencherTabelaOpRanking("opRankDsWorst", ds.piores, false, vazio(acumDs));

  const nota = document.getElementById("opRankNota");
  if (nota) {
    if (!temDados(acumSla) && !temDados(acumDs)) nota.textContent = "";
    else if (ignorarMinimo) nota.textContent = "Filtro por entregador: os rankings mostram só ele (sem mínimo de pacotes).";
    else nota.textContent = `Rankings por entregador, somando os dias do período. Só entram entregadores com pelo menos ${OP_RANKING_MIN_PACOTES} pacotes.`;
  }
}

// ------------------------------------------------------------
// Ranking de PNR por entregador (Análise de OP) — fica ao lado do
// gráfico de PNR. Conta as PNRs EM ABERTO de cada entregador no
// último dia do período que tem arquivo de PNR:
//   • 10 piores  = quem tem MAIS PNR em aberto
//   • 10 melhores = quem tem MENOS (zero é o melhor). Os zerados vêm
//     dos entregadores do SLA/DS do período, então só entram os que
//     têm pelo menos OP_RANKING_MIN_PACOTES pacotes (igual aos outros).
// Respeita os filtros de cidade/entregador da aba.
// ------------------------------------------------------------
function chaveNomeOp(nome) {
  return String(nome || "")
    .normalize("NFD")
    .replace(/[\u0300-\u036f]/g, "")
    .toLowerCase()
...
// Conta as PNRs em aberto de um dia, por entregador
function contarPnrAbertasPorEntregadorOp(dia, cityValue, driverValue) {
  const slaRaw = (dia.SLA || []).flatMap((f) => parseCsvString(f.conteudo));
  const dsRaw = (dia.DS || []).flatMap((f) => parseCsvString(f.conteudo));
  const pnrRaw = (dia.PNR || []).flatMap((f) => parseCsvString(f.conteudo));

  // cidade de cada pedido (pelo CEP do SLA/DS do mesmo dia)
  const cidadePorPedido = {};
  [...slaRaw, ...dsRaw].forEach((r) => {
    const id = (r["Order ID"] || "").toString().trim();
    const cidade = cepToCity[r["Postal Code"]];
    if (id && cidade && !cidadePorPedido[id]) cidadePorPedido[id] = cidade;
  });

  const porChave = {};
  pnrRaw.forEach((r) => {
    if (!OPEN_PNR_STATUSES.includes((r["Status"] || "").toString().trim())) return;

    const nome = extractHandlerName(r["Driver"] || r["Driver Name"]) || "Sem responsável";
    if (driverValue && chaveNomeOp(nome) !== chaveNomeOp(driverValue)) return;
    if (cityValue && cidadePorPedido[(r["SPXTN"] || "").toString().trim()] !== cityValue) return;

    const chave = chaveNomeOp(nome);
    if (!porChave[chave]) porChave[chave] = { name: nome, abertas: 0, valor: 0 };
    porChave[chave].abertas += 1;
    porChave[chave].valor += parseFloat(r["PNR Order Value"]) || 0;
  });

  return { porChave, temArquivo: pnrRaw.length > 0 };
}

function montarRankingPnrOp(porChave, acumSla, acumDs, ignorarMinimo) {
  // entregadores do SLA/DS do período (o volume serve pra desempatar e pro mínimo)
  const universo = {};
  [acumSla, acumDs].forEach((acum) => {
    Object.entries(acum || {}).forEach(([name, s]) => {
      const chave = chaveNomeOp(name);
      if (!chave) return;
      if (!universo[chave]) universo[chave] = { name, volume: 0 };
      universo[chave].volume = Math.max(universo[chave].volume, s.total);
    });
  });

  const chaves = new Set([...Object.keys(universo), ...Object.keys(porChave)]);
  const lista = [...chaves].map((chave) => {
    const pnr = porChave[chave];
    const uni = universo[chave];
    return {
      name: (uni && uni.name) || (pnr && pnr.name) || chave,
      abertas: pnr ? pnr.abertas : 0,
      valor: pnr ? pnr.valor : 0,
      volume: uni ? uni.volume : 0,
    };
  });

  const porNome = (a, b) => a.name.localeCompare(b.name, "pt-BR");

  // piores: mais PNR em aberto primeiro (empate: maior valor em risco)
  const piores = lista
    .filter((r) => r.abertas > 0)
    .sort((a, b) => b.abertas - a.abertas || b.valor - a.valor || porNome(a, b))
    .slice(0, OP_RANKING_TOP);

  // melhores: menos PNR em aberto primeiro, zero é o melhor
  // (empate: quem entregou mais pacotes fica na frente)
  const melhores = lista
    .filter((r) => r.volume > 0 && (ignorarMinimo || r.volume >= OP_RANKING_MIN_PACOTES))
    .sort((a, b) => a.abertas - b.abertas || b.volume - a.volume || porNome(a, b))
    .slice(0, OP_RANKING_TOP);

  return { piores, melhores };
}

function preencherTabelaOpPnr(tbodyId, lista, tipo, mensagemVazia) {
  const tbody = document.getElementById(tbodyId);
  if (!tbody) return;

  if (!lista.length) {
    tbody.innerHTML = `<tr><td colspan="4" class="table-empty">${mensagemVazia}</td></tr>`;
    return;
  }
  ...
  tbody.innerHTML = lista
    .map((r, i) => {
      const nome = escaparHtmlOp(r.name);
      const classe = r.abertas === 0 ? "op-pnr-bom" : "op-pnr-ruim";
      const ultima = tipo === "piores"
        ? r.valor.toLocaleString("pt-BR", { style: "currency", currency: "BRL" })
        : r.volume;
      return `<tr>
        <td class="op-rank-pos">${tipo === "melhores" && i < 3 ? medalhas[i] : i + 1}</td>
        <td title="${nome}">${nome}</td>
        <td class="op-pnr-num ${classe}">${r.abertas}</td>
        <td>${ultima}</td>
      </tr>`;
    })
    .join("");
...
function renderOpPnrRankings(diasDoPeriodo, acumSla, acumDs, cityValue, driverValue) {
  const nota = document.getElementById("opPnrRankNota");

  // último dia do período que tem arquivo de PNR (sem arquivo não dá pra
  // saber quem tem zero — todo mundo pareceria "o melhor")
  let dia = null;
  for (let i = (diasDoPeriodo || []).length - 1; i >= 0; i--) {
    if ((diasDoPeriodo[i].PNR || []).length) { dia = diasDoPeriodo[i]; break; }
  }

  if (!dia) {
    preencherTabelaOpPnr("opRankPnrWorst", [], "piores", "Sem arquivo de PNR no período");
    preencherTabelaOpPnr("opRankPnrBest", [], "melhores", "Sem arquivo de PNR no período");
    if (nota) nota.textContent = "";
    return;
  }

  const { porChave } = contarPnrAbertasPorEntregadorOp(dia, cityValue, driverValue);
  const { piores, melhores } = montarRankingPnrOp(porChave, acumSla, acumDs, !!driverValue);

  preencherTabelaOpPnr("opRankPnrWorst", piores, "piores", "Nenhuma PNR em aberto 🎉");
  preencherTabelaOpPnr(
    "opRankPnrBest",
    melhores,
    "melhores",
    Object.keys(acumSla || {}).length || Object.keys(acumDs || {}).length
      ? `Nenhum entregador com pelo menos ${OP_RANKING_MIN_PACOTES} pacotes no período`
      : "Sem dados de SLA/DS no período"
  );

  if (nota) {
    nota.textContent =
      `Ranking de PNR: conta as PNRs em aberto em ${formatarDataBR(dia.data)} (último dia do período com arquivo de PNR). ` +
      `Quem tem mais PNR em aberto é o pior; zero é o melhor.`;
  }
}
...
async function gerarAnaliseOp() {
  ...
  // acumulado por entregador (SLA e DS) pros rankings do período
  const acumRankSla = {};
  const acumRankDs = {};
  ...
  const pontos = diasDoPeriodo.map((dia) => {
    ...
    acumularRankingOp(acumRankSla, slaMetrics.driverSLA);
    acumularRankingOp(acumRankDs, dsMetrics.driverSLA);
  ...
  renderOpRankings(acumRankSla, acumRankDs, driverValue);
  renderOpPnrRankings(diasDoPeriodo, acumRankSla, acumRankDs, cityValue, driverValue);
}
